...
def create_cfn_template(environment, region):
    ...
    # If the subnets are private, do this instead
        ...
        for index, az in enumerate(availability_zones[region]):
            i = index + 1  # Dont want zero-indexed names
            publiccidr = FindInMap(
                'CidrAllocations', region, f'publicsubnet{i}'
            )
            privatecidr = FindInMap(
                'CidrAllocations', region, f'privatesubnet{i}'
            )
            ...
            publicsubnet = t.add_resource(
                ec2.Subnet(
                    ...
                    AvailabilityZone=az,
                    Tags=default_tags
                    + Tags(Name=f'{environment}-public-subnet-{i}'),
                )
            )
            ...
            privatesubnet = t.add_resource(
                ec2.Subnet(
                    ...
                    AvailabilityZone=az,
                    Tags=default_tags
                    + Tags(Name=f'{environment}-private-subnet-{i}'),
                )
            )
            ...
            privatesubnetroutetable = t.add_resource(
                ec2.RouteTable(
                    ...
                    VpcId=Ref(vpc),
                    Tags=default_tags
                    + Tags(Name=f'{environment}-private{i}-rtb'),
                )
            )
