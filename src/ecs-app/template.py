...
### Security group vars
sg_ingress_rules = []
sg_ingress_rules += sg_allow_from(
    {
        'public': '0.0.0.0/0'
    },
    2456,
...
environment_variables = [
    ...
    ecs.Environment(Name='PRE_START_HOOK',
    ...
    ecs.Environment(Name='POST_START_HOOK',
    ...
    ecs.Environment(Name='VALHEIM_LOG_FILTER_CONTAINS_Spawned', Value='Got character ZDOID from'),
    ecs.Environment(Name='ON_VALHEIM_LOG_FILTER_CONTAINS_Spawned',
...
def create_cfn_template(environment, region):
    ...
    outboundacl = t.add_resource(ec2.NetworkAclEntry(
        'outboundacl',
        NetworkAclId=Ref(networkacl),
        RuleNumber='100',
        Protocol='-1',
    ...
    #### Start EFS resources
    efsfilesystem = t.add_resource(efs.FileSystem(
        'efsfilesystem',
        FileSystemTags=default_tags
    ))
    ...
    #### Start NLB resources
    networkloadbalancer = t.add_resource(elb.LoadBalancer(
        "networkloadbalancer",
        Type='network',
    ...
    udp2456tg = t.add_resource(elb.TargetGroup(
        "udp2456tg",
        Port=2456,
        TargetType='ip',
        HealthCheckPort='80',
    ...
    udp2456listener = t.add_resource(elb.Listener(
        "udp2456listener",
        Port="2456",
        Protocol="UDP",
    ...
    udp2457tg = t.add_resource(elb.TargetGroup(
        "udp2457tg",
        Port=2457,
        TargetType='ip',
        Protocol="UDP",
    ...
    udp2457listener = t.add_resource(elb.Listener(
        "udp2457listener",
        Port="2457",
        Protocol="UDP",
        LoadBalancerArn=Ref(networkloadbalancer),
    ...
    # Configure the app target group for udp 2457
    udp2458tg = t.add_resource(elb.TargetGroup(
        "udp2458tg",
        Port=2458,
        TargetType='ip',
        Protocol="UDP",
    ...
    udp2458listener = t.add_resource(elb.Listener(
        "udp2458listener",
        Port="2458",
        Protocol="UDP",
        LoadBalancerArn=Ref(networkloadbalancer),
    ...
    # Configure the app target group for udp 2457
    tcp80tg = t.add_resource(elb.TargetGroup(
        "tcp80tg",
        Port=80,
        TargetType='ip',
        Protocol="TCP",
    ...
    tcp80listener = t.add_resource(elb.Listener(
        "tcp80listener",
        Port="80",
        Protocol="TCP",
        LoadBalancerArn=Ref(networkloadbalancer),
